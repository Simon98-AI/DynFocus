...
class LLaVATrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_modality_length:
            ...
            print(lengths, self.args.train_batch_size, self.args.world_size)
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                ...
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector.bin')) # 只保存mm_projector这个部分
        else:
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(LLaVATrainer, self)._save(output_dir, state_dict) # 其中这个函数额外保存一些tokenizer：self.tokenizer.save_pretrained(output_dir)
        # print(self.args.save_only_model)
        # print("------------------------------")
