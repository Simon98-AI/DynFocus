...
def annotate(prediction_set, caption_files, output_dir):
    ...
    for file in caption_files:
        ...
        try:
            ...
            response_message = model.generate(messages)
...
def main():
    ...
    # Parse arguments.
    args = parse_args()
    # collect all the json file
    pred_contents = []
    cnt = 0
    for idx in range(args.num_chunks):
        json_file = open(os.path.join(args.pred_path, f"{args.num_chunks}_{idx}.json"), 'r', encoding='utf-8')
        for item in json_file.readlines():
            pred_contents.append(json.loads(item))
            # try:
            #     pred_contents.append(json.loads(item))
    ...
    for file_name in os.listdir(output_dir):
        if file_name.endswith(".json"):
            ...
            with open(file_path, "r") as json_file:
                #print(file_path, "pinpoint to it")
